...
class SystemTB : public System {
    ...
    public:  // Const references to attributes (read-only)
        // List of direct lattice vectors {Ri} in Angstrom associated to the stored Hamiltonian matrices, stored by columns: (3,ncells).
        ...
        // List of k-points in Angstrom^-1 employed in the BSE Hamiltonian that define the exciton basis, stored by (3D) columns
    ...
    public:
        ...
        /* Bloch Hamiltonian */
        arma::cx_mat hamiltonian(const arma::colvec& k) const;
        arma::cx_mat overlap(const arma::colvec& k) const;
        // using System::solveBands;
        void solveBands(const arma::colvec&, arma::colvec&, arma::cx_mat&) override;
