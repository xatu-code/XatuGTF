 ...
 */
IntegralsDipole_MPI::IntegralsDipole_MPI(const IntegralsBase& IntBase, const int procMPI_rank, const int procMPI_size, const int tol, const uint32_t nR, const std::string& intName) : IntegralsBase{IntBase} {

    dipolefun(procMPI_rank, procMPI_size, tol, nR, intName);
 ...
 */
void IntegralsDipole_MPI::dipolefun(const int procMPI_rank, const int procMPI_size, const int tol, const uint32_t nR, const std::string& intName){

const double PIpow = std::pow(PI,1.5);
arma::mat combs;
arma::mat RlistAU = ANG2AU*generateRlist(nR, combs, "Dipole", procMPI_rank);  //convert Bravais vectors from Angstrom to atomic units
...
auto begin = std::chrono::high_resolution_clock::now();  
    ...
    dipoleMatrices.reserve(2*(elems_per_proc + 1));  // the factor 2 accounts for the upper triangle, reconstructed from the lower

    for(uint64_t s = procMPI_rank*elems_per_proc + p1; s < (procMPI_rank+1)*elems_per_proc + p2; s++){ //Spans the lower triangle of all the nR_star matrices <mu,0|r|mu',R>
...
uint64_t n_entries_thisproc = dipoleMatrices.size();
uint64_t n_entries_total;
MPI_Reduce(&n_entries_thisproc, &n_entries_total, 1, MPI_UINT64_T, MPI_SUM, 0, MPI_COMM_WORLD);

std::string dipstr = IntFiles_Dir + "dipoleMat_" + intName + ".dip";
if(procMPI_rank == 0){
    std::ofstream output_file0(dipstr, std::ios::trunc);
    output_file0.close();
    std::ofstream output_file(dipstr, std::ios::app);
    output_file << "DIPOLE INTEGRALS" << std::endl;
    output_file << "Requested nR: " << nR << ". Computed nR: " << nR_star << std::endl;
    output_file << "Tolerance: 10^-" << tol << ". Matrix density: " << ((double)n_entries_total/(dimMat_SCF*dimMat_SCF*nR_star))*100 << " %" << std::endl;
