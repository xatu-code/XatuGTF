 ...
 * The resulting cube (third dimension spans the Bravais vectors) is saved in the o2MixMat_intName.o2mc, and the list of 
 * Bravais vectors in fractional coordinates is saved in the RlistFrac_intName.o2mc file
...
arma::mat combs;
arma::mat RlistAU = ANG2AU*generateRlist(nR, combs, "Overlap2MixC");
...
auto begin = std::chrono::high_resolution_clock::now();  
    ...
    for(uint64_t s = 0; s < total_elem; s++){ //Spans all the elements of all the nR_star matrices <P,0|mu,R>
        ...
        uint32_t Rind {static_cast<uint32_t>(s / nelem_rectang)};   //Position in RlistAU (e.g. 0 for R=0) of the corresponding Bravais vector 
        uint32_t orb_bra {static_cast<uint32_t>(sind % dimMat_AUX_)};  //Orbital number (<dimMat_AUX) of the bra corresponding to the index s
        uint32_t orb_ket {static_cast<uint32_t>(sind / dimMat_AUX_)};  //Orbital number (<dimMat_SCF) of the ket corresponding to the index s
        ...
        int L_ket  {orbitals_info_int_SCF_[orb_ket][2]};
        int m_ket  {orbitals_info_int_SCF_[orb_ket][3]};
        int nG_ket {orbitals_info_int_SCF_[orb_ket][4]};
        arma::colvec coords_ket {RlistAU.col(Rind) + arma::colvec{orbitals_info_real_SCF_[orb_ket][0], orbitals_info_real_SCF_[orb_ket][1], orbitals_info_real_SCF_[orb_ket][2]} };  //Position (a.u.) of ket atom
        ...
        double FAC12_braket = FAC12_AUX_[orb_bra]*FAC12_SCF_[orb_ket];
        ...
        for(int gaussC_bra = 0; gaussC_bra < nG_bra; gaussC_bra++){ //Iterate over the contracted Gaussians in the bra orbital
            ...
            for(int gaussC_ket = 0; gaussC_ket < nG_ket; gaussC_ket++){ //Iterate over the contracted Gaussians in the ket orbital
                double exponent_ket {orbitals_info_real_SCF_[orb_ket][2*gaussC_ket + 3]};
                ...
                overlap_g_pre *= FAC3_AUX_[orb_bra][gaussC_bra]*FAC3_SCF_[orb_ket][gaussC_ket]*std::pow(p,-1.5)*std::exp(-exponent_bra*exponent_ket*norm_braket/p);
...
combs.save(IntFiles_Dir + "RlistFrac_" + intName + ".o2mc", arma::arma_ascii);
...
std::cout << "Matrices stored in the file: " << IntFiles_Dir + "o2MixMat_" + intName + ".o2mc" << 
    " , and list of Bravais vectors in " << IntFiles_Dir + "RlistFrac_" + intName + ".o2mc" << std::endl;
