 ...
 */
IntegralsCoulomb2C_MPI::IntegralsCoulomb2C_MPI(const IntegralsBase& IntBase, const int procMPI_rank, const int procMPI_size, const int tol, const std::vector<int32_t>& nRi, const std::string& intName) : IntegralsBase{IntBase} {

    Coulomb2Cfun(procMPI_rank, procMPI_size, tol, nRi, intName);
 ...
 */
void IntegralsCoulomb2C_MPI::Coulomb2Cfun(const int procMPI_rank, const int procMPI_size, const int tol, const std::vector<int32_t>& nRi, const std::string& intName){

const double PIpow = std::pow(PI,2.5);
arma::mat combs;
arma::mat RlistAU = ANG2AU*generateRlist_fixed(nRi, combs, "Coulomb2C", procMPI_rank);
...
auto begin = std::chrono::high_resolution_clock::now();  
    ...
    Coulomb2Matrices.reserve(elems_per_proc + 1);
...
uint64_t n_entries_thisproc = Coulomb2Matrices.size();
uint64_t n_entries_total;
MPI_Reduce(&n_entries_thisproc, &n_entries_total, 1, MPI_UINT64_T, MPI_SUM, 0, MPI_COMM_WORLD);

std::string C2Cstr = IntFiles_Dir + "C2Mat_" + intName + ".C2c";
if(procMPI_rank == 0){
    std::ofstream output_file0(C2Cstr, std::ios::trunc);
    output_file0.close();
    std::ofstream output_file(C2Cstr, std::ios::app);
