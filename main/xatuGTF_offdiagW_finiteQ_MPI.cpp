...
int main(int argc, char* argv[]){
    ...
    if(procMPI_rank == 0){
        ...
        std::cout << std::endl;
        std::cout << "+---------------------------------------------------------------------------+" << std::endl;
        std::cout << "|                          BSE k/=k' TERMS (Q!=0)                           |" << std::endl;
        std::cout << "+---------------------------------------------------------------------------+" << std::endl;
        std::cout << "WARNING: this is a temporary version, not very efficient. For the Q=0 case, use the other script" << std::endl;
    }
    ...
    std::string savefile = "custom";       //Result will be saved in file Results/2-BSEHamiltonian/savefile.offdiag
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(procMPI_rank == 0){
        ...
        arma::cout << "Q in FRACTIONAL COORDINATES: " << GTFexc.Q.t() << arma::endl;
    }
    ...
    for(uint32_t chunk = 0; chunk < nkchunks_thisproc; chunk++){ // iterate over chunks (sets in bijection with all the different |k-k'|)
        ...
        for(uint32_t kpair = 0; kpair < kentries_chunks_thisproc[chunk].size(); kpair++){ // iterate over all the (k,k') pairs within the present chunk (i.e. with the same |k-k'|)       
            ...
            arma::colvec k1Q = k1 + GTFexc.Q;
            arma::colvec k2Q = k2 + GTFexc.Q;
            bool k_plus_minus = kentries_chunks_thisproc[chunk][kpair][2];

            arma::colvec eigval1, eigval2, eigval1Q, eigval2Q;
            arma::cx_mat eigvec1, eigvec2, eigvec1Q, eigvec2Q; 
            GTFexc.solveBands(k1, eigval1, eigvec1);
            GTFexc.solveBands(k2, eigval2, eigvec2);
            GTFexc.solveBands(k1Q, eigval1Q, eigvec1Q);
            GTFexc.solveBands(k2Q, eigval2Q, eigvec2Q);
            ...
            arma::cx_mat eigvec1vQ = eigvec1Q.cols(GTFexc.valenceBands);
            arma::cx_mat eigvec2vQ = eigvec2Q.cols(GTFexc.valenceBands);
            arma::cx_mat eigvec1cQ = eigvec1Q.cols(GTFexc.conductionBands);
            arma::cx_mat eigvec2cQ = eigvec2Q.cols(GTFexc.conductionBands);

            arma::cx_mat eiKR2mat  = arma::zeros<arma::cx_mat>(GTFexc.ncellsM3c + 1,GTFexc.ncellsM3c + 1);
            arma::cx_mat eiKR2matQ = arma::zeros<arma::cx_mat>(GTFexc.ncellsM3c + 1,GTFexc.ncellsM3c + 1);
            for(uint i1 = 0; i1 <= GTFexc.ncellsM3c; i1++){
                double k1R1 = arma::dot(k1, GTFexc.RlistFrac_M3c.col(i1));
                double k1R1Q = arma::dot(k1Q, GTFexc.RlistFrac_M3c.col(i1));
                for(uint i2 = 0; i2 <= GTFexc.ncellsM3c; i2++){
                    ...
                    eiKR2matQ(i1,i2) = std::exp( GTFexc.imag*TWOPI*(arma::dot(k2Q, GTFexc.RlistFrac_M3c.col(i2)) - k1R1Q) );
                }
            }

            arma::cx_mat vPmuk1nuk2_pre  = arma::zeros<arma::cx_mat>(GTFexc.norbitals_AUX, GTFexc.norbitals*GTFexc.norbitals);
            arma::cx_mat vPmuk1nuk2_preQ = arma::zeros<arma::cx_mat>(GTFexc.norbitals_AUX, GTFexc.norbitals*GTFexc.norbitals);
            for(uint64_t i = 0; i < GTFexc.nvaluesM3c; i++){
                ...
                vPmuk1nuk2_preQ(GTFexc.metric3CIndices[i][0], GTFexc.metric3CIndices[i][1] + GTFexc.norbitals*GTFexc.metric3CIndices[i][2]) += GTFexc.metric3CValues[i] * eiKR2matQ(GTFexc.metric3CIndices[i][3], GTFexc.metric3CIndices[i][4]);
            }

            if(k_plus_minus == k0_plus_minus){  // k-k' = k_0 - k'_0
                vPmuk1nuk2_pre  = JMprod_conj * vPmuk1nuk2_pre;
                vPmuk1nuk2_preQ = JMprod_conj * vPmuk1nuk2_preQ;
                arma::cx_mat vPvv = ( vPmuk1nuk2_pre * arma::kron(eigvec2v, arma::conj(eigvec1v)) ).st();
                arma::cx_mat vPcc = vPmuk1nuk2_preQ * arma::kron(eigvec2cQ, arma::conj(eigvec1cQ));
                ...
                vPmuk1nuk2_preQ = JMprod * vPmuk1nuk2_preQ;
                arma::cx_mat vPvv = ( vPmuk1nuk2_pre * arma::kron(eigvec2v, arma::conj(eigvec1v)) ).st();
                arma::cx_mat vPcc = vPmuk1nuk2_preQ * arma::kron(eigvec2cQ, arma::conj(eigvec1cQ));
                for(uint s = 0; s < dimkblock_square; s++){
                    uint i = s % GTFexc.dimkblock;
    ...
    if(procMPI_rank == 0) { // first clear the corresponding file if already present, then write the header
        ...
        output_file << "NON-DIAGONAL W BLOCKS, Q = (" << GTFexc.Q(0) << ", " << GTFexc.Q(1) << ", " << GTFexc.Q(2) << ")" << std::endl;
